...
#Definition der Datenbank (MariaDB)
...
"""
Benutzer*: appCarFuel
Passwort*: X690!6GM2fK6iF8lH6
Host: dbhost (Mariadb Container Name)
Port: 3306
Datenbank: carFuel
"""

#Erstellung Datenbankobjekt 
db = SQLAlchemy(app)

#Definition der Basis-URL
base_url = "http://20.127.45.181:5000"


#Definition diverser Klassen
class User(Base):
    #Erstellung DB-Tabelle user
    __tablename__ = 'user'
...
class Transaction(Base):
    #Erstellung DB-Tabelle transaction
...
#Erstellung Erstellung der Tabellen
...
def user():
    if request.method == 'POST':
        ...
        try:
            ...
            session.close()
            return make_response({"key": "Erfolgreich hinzugefügt"}, 200)
        except Exception as e:
            print(str(e))
            return make_response({"key": "Benutzername bereits vorhanden"}, 400)
...
def login():
    if request.method == "POST":
        ...
        if user:
            ...
            return make_response({"key": "Anmeldung erfolgreich"}, 200)
        else:
            return make_response({"key": "Falsche Email oder Passwort"}, 400)
    else:
        if 'user' in session:
            return redirect('/')
        else:
